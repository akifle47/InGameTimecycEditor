...
static void ImGui_ImplDX9_SetupRenderState(ImDrawData* draw_data)
{
    ...
    device->SetVertexShader(bd->pVS);
    device->SetPixelShader(bd->pPS);
...
void ImGui_ImplDX9_RenderDrawData(ImDrawData* draw_data)
{
    ...
    if (!bd->pVB || bd->VertexBufferSize < draw_data->TotalVtxCount)
    {
        ...
        if (device->CreateVertexBuffer((UINT)bd->VertexBufferSize * sizeof(CUSTOMVERTEX), D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, NULL, D3DPOOL_DEFAULT, &bd->pVB, nullptr) < 0)
            return;
    }
    ...
    device->SetVertexDeclaration(bd->pVD);
...
bool ImGui_ImplDX9_CreateDeviceObjects()
{
    ...
    hr = bd->pd3dDevice->CreateVertexShader((DWORD*)vertex_shader_buffer->GetBufferPointer(), &bd->pVS);
    if(hr != D3D_OK)
    {
        return false;
    }
    ...
    hr = bd->pd3dDevice->CreatePixelShader((DWORD*)pixel_shader_buffer->GetBufferPointer(), &bd->pPS);
    if(hr != D3D_OK)
    {
        return false;
    }
    ...
    HRESULT declHr = bd->pd3dDevice->CreateVertexDeclaration(vertexDeclElements, &bd->pVD);
    if(declHr != D3D_OK)
    {
        return false;
    }
...
void ImGui_ImplDX9_InvalidateDeviceObjects()
{
    ...
    if (bd->pVB) { bd->pVB->Release(); bd->pVB = nullptr; }
    if (bd->pIB) { bd->pIB->Release(); bd->pIB = nullptr; }
    if (bd->pVD) { bd->pVD->Release(); bd->pVD = nullptr; }
    if (bd->pVS) { bd->pVS->Release(); bd->pVS = nullptr; }
    if (bd->pPS) { bd->pPS->Release(); bd->pPS = nullptr; }
}
